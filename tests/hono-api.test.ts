...
import { HONO_PORT } from './setup';

describe('Hono API Tests', () => {
    const baseUrl = `http://localhost:${HONO_PORT}`;
    ...
    describe('Hono vs Express Compatibility', () => {
        test('should produce identical responses to Express version', async () => {
            ...
            const response = await fetch(`${baseUrl}/api/v1/public/ping`);
            const expressResponse = await fetch(`http://localhost:3001/api/v1/public/ping`);
        ...
        test('should handle query parameters identically', async () => {
            ...
            const expressResponse = await fetch(`http://localhost:3001/api/v1/public/status/probe1?match=true`);
